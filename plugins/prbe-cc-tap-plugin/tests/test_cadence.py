...
def _isolated_plugin_dir(monkeypatch):
    tmp = tempfile.mkdtemp(prefix="prbe-cc-tap-cadence-test-")
    monkeypatch.setenv("PRBE_CC_TAP_PLUGIN_DIR", tmp)
    # Clear any inherited interval env vars so each test starts clean.
    for var in (
        "PRBE_CC_TAP_INTERVAL_SECONDS",
        "PRBE_CC_TAP_ACTIVE_INTERVAL_SECONDS",
        "PRBE_CC_TAP_IDLE_INTERVAL_SECONDS",
    ):
...
def test_intervals_legacy_env_overrides_both(
    ...
    """PRBE_CC_TAP_INTERVAL_SECONDS=120 means flat 120s for active + idle —
    no adaptive switching."""
    from tap import config as cfg

    monkeypatch.setenv("PRBE_CC_TAP_INTERVAL_SECONDS", "120")
...
def test_intervals_env_overrides_config(
    ...
    _write_config(_isolated_plugin_dir, {"active_interval_seconds": 30})
    monkeypatch.setenv("PRBE_CC_TAP_ACTIVE_INTERVAL_SECONDS", "45")
